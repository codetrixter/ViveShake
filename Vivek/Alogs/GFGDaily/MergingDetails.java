...
public class MergingDetails {
    ...
    public static void main(String[] args) {
        ...
        for (List<String> d : new MergingDetails().mergeDetails(details)) {
    ...
    public List<List<String>> mergeDetails(List<List<String>> details) {
        // Your code here
        HashMap<Integer, HashSet<String>> emails = new HashMap<>();
        ...
        for (int i = 0; i < details.size(); i++) {
            ...
            for (int j = 1; j < list.size(); j++) {
                if (emailMap.getOrDefault(list.get(j), -1) != -1) {
                    found = true;
                    group = emailMap.get(list.get(j));
                    break;
                }
            }
            ...
            if (found) {
                for (int j = 1; j < list.size(); j++) {
                    HashSet<String> set = emails.getOrDefault(group, new HashSet<>());
                    set.add(list.get(j));
                    emailMap.put(list.get(j), group);
                    emails.put(group, set);
                }

            } else {
                for (int j = 1; j < list.size(); j++) {
                    emailMap.put(list.get(j), i);
                    HashSet<String> set = emails.getOrDefault(i, new HashSet<String>());
                    set.add(list.get(j));
                    emails.put(i, set);
                }
            }
        ...
        for (Integer index : emails.keySet()) {
            List<String> emls = new ArrayList<>(emails.get(index));
            Collections.sort(emls);
            emls.add(0, details.get(index).get(0));
            res.add(emls);
        }
        return res;
    }
